...
namespace Test.UnitTests
{
    public class TestMultiTenantDbContext
    {
        ...
        public void TestShopHierarchicalFilterOk(int shopKey, string districtManagerId, string stockName)
        {
            ...
            using (var context = new MultiTenantDbContext(options, new MockGetClaimsProvider("user-id", 1, "manager-id")))
            {

                //ATTEMPT
                var filtered = context.CurrentStock.IgnoreQueryFilters().Where(x => districtManagerId == null
                    ? x.ShopKey == shopKey
                    : x.DistrictManagerId == districtManagerId).ToList();
